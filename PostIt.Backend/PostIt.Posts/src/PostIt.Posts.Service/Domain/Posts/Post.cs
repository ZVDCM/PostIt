...
public sealed class Post : Entity, IAuditable
{
    ...
    private Post(PostId id, UserId userId, string username, string title, string image)
    {
        Id = id;
        UserId = userId;
        Username = username;
    ...
    public static Post Create(Guid userId, string username, string title, string image)
        => new(new PostId(Guid.NewGuid()), new UserId(userId), username, title, image);
